...
# 设置工作路径
os.chdir("D:\\R")  # 直接设置成桌面上的TCMSP文件夹就好了
...
def TCMSP(herb_name):
    ...
    # 添加其他必要的选项，如果需要
    # options.add_argument('--other-options')
    ...
    # 获取网页数据
    try:
        driver.get(f'https://www.tcmsp-e.com/tcmspsearch.php?qs=herb_all_name&q={herb_name}&token={token}')
    ...
    # 获取页码
    ingredients_page = int(driver.find_element(By.XPATH,
                                               '//div[@id="grid"]//div[@data-role="pager"]//a[@title="Go to the last page"]').get_attribute(
        "data-page"))
    targets_page = int(driver.find_element(By.XPATH,
                                           '//div[@id="grid2"]//div[@data-role="pager"]//a[@title="Go to the last page"]').get_attribute(
        "data-page"))
    ...
    for _ in range(ingredients_page):
        ...
        btn_ing = driver.find_element(By.XPATH,
                                      '//div[@id="grid"]//div[@data-role="pager"]//a[@title="Go to the next page"]')  # 翻页
        btn_ing.click()
    ...
    for _ in range(targets_page - 1):
        ...
        btn_ing1 = driver.find_element(By.XPATH,
                                       '//div[@id="grid2"]//div[@data-role="pager"]//a[@title="Go to the next page"]')  # 翻页
        btn_ing1.click()
        time.sleep(random.uniform(2, 3))  # 随机等待2-5秒
    driver.quit()
